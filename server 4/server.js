...
function formRoom(family, variant, type, group){
  ...
  // 暗棋這種要洗牌的玩法，雙方必須看到同一副牌。
  // 伺服器發一個種子，兩邊用同樣種子做確定性洗牌就會一致，
  // 不用把整副牌傳出去（傳了等於讓人直接看光暗棋）。
  const seed = (Math.random() * 0x7fffffff) | 0;
  rooms[gameId] = { family, variant, type, seats, dbGameId: dbGame.id, seed };
...
function broadcastRoom(io, room){
  ...
  io.to('room:'+room.code).emit('room:update', {
    ...
    need: room.need,
    hostId: room.hostId,
    members: room.members.map(m=>({ socketId:m.socketId, name:m.name })),
    canStart: room.members.length === room.need
  });
}

function destroyRoomIfEmpty(code){
  const room = privateRooms[code];
  if(room && room.members.length === 0) delete privateRooms[code];
}
...
io.on('connection', (socket)=>{
  ...
  // 暗棋走法轉發（翻牌 / 移動 / 放鬼）
  socket.on('bqmove', ({ gameId, action })=>{
    if(!gameId || !rooms[gameId]) return;
    db.bumpMoveCount(rooms[gameId].dbGameId);
    socket.to(gameId).emit('bqmove', { action, fromSeat: seatOf(gameId, socket.id) });
  });
